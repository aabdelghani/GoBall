...
static void get_video_dimensions(const char *path)
{
    ...
    if (fp)
    {
        ...
        {
            DEBUG_WARN(MODULE_VIDEO, "ffprobe could not detect dimensions, using %dx%d", _src_w,
                       _src_h);
        }
        pclose(fp);
    }
...
static void get_video_duration(const char *path)
{
    ...
    {
        DEBUG_ERROR(MODULE_VIDEO, "Failed to run ffprobe (is ffmpeg installed?): %s",
                    strerror(errno));
    }
}

/* Move ffplay window and refocus app (Wayland/labwc).
 * Uses double-fork so grandchild can sleep without blocking LVGL. */
static void move_and_refocus_async(int x, int y)
{
    pid_t p = fork();
    if (p == 0)
    {
        ...
        if (fork() == 0)
        {
            /* Grandchild: wait for ffplay to appear, then move + refocus */
            usleep(800000); /* 800ms */
            char cmd[256];
            snprintf(cmd, sizeof(cmd),
                     "wlrctl toplevel move %d %d title:ffplay; "
                     "wlrctl toplevel focus goball",
                     x, y);
...
static void spawn_ffplay(double start_sec)
{
    ...
    if (video_pid == 0)
    {
        ...
        if (start_sec > 0.5)
        {
            execlp("ffplay", "ffplay", "-noborder", "-alwaysontop", "-x", width_s, "-y", height_s,
                   "-left", left_s, "-top", top_s, "-ss", ss_s, "-loglevel", "quiet", _video_path,
                   NULL);
        }
        else
        {
            execlp("ffplay", "ffplay", "-noborder", "-alwaysontop", "-x", width_s, "-y", height_s,
                   "-left", left_s, "-top", top_s, "-loglevel", "quiet", _video_path, NULL);
        }
    ...
    else
    {
        ...
        DEBUG_DEBUG(
            MODULE_VIDEO,
            "ffplay cmd: ffplay -noborder -alwaysontop -x %d -y %d -left %d -top %d %s%s %s",
            _video_w, _video_h, _screen_x, _screen_y, start_sec > 0.5 ? "-ss " : "",
            start_sec > 0.5 ? "N" : "", _video_path);
        /* Move ffplay to correct position and refocus app window */
        move_and_refocus_async(_screen_x, _screen_y);
    }
}
...
static void btn_pause_cb(lv_event_t *e)
{
    ...
    if (video_pid > 0)
    {
        if (_paused)
        {
            DEBUG_DEBUG(MODULE_VIDEO, "Resume (PID %d, offset=%.1fs)", (int)video_pid,
                        _seek_offset);
        ...
        else
        {
            ...
            DEBUG_DEBUG(MODULE_VIDEO, "Pause (PID %d, position=%.1fs)", (int)video_pid,
                        _seek_offset);
...
void game_video_play(lv_obj_t *parent, const char *video_path, game_video_back_cb_t back_cb)
{
    ...
    if (video_pid > 0)
    {
        DEBUG_DEBUG(MODULE_VIDEO, "Stopping previous video (PID %d) before starting new one",
                    (int)video_pid);
        game_video_stop();
    }
    ...
    DEBUG_INFO(MODULE_VIDEO, "LVGL panel coords: x=%d y=%d w=%d h=%d", lvgl_x, lvgl_y, lvgl_w,
               lvgl_h);
    ...
    DEBUG_DEBUG(MODULE_VIDEO, "Panel: pos(%d,%d) size(%dx%d), controls_height=%d", panel_x, panel_y,
                panel_w, panel_h, CONTROLS_HEIGHT);
    ...
    /* Place video inside the panel's usable area (centered). */
    int x_offset = (inner_w - fit_w) / 2;
    int y_offset = (avail_h - fit_h) / 2;
    _screen_x    = win_x + inner_x + x_offset;
    _screen_y    = win_y + inner_y + y_offset;

    DEBUG_INFO(MODULE_VIDEO, "Video rect: screen(%d,%d) size(%dx%d)", _screen_x, _screen_y,
               _video_w, _video_h);
    DEBUG_INFO(MODULE_VIDEO, "  Panel outer: x=%d y=%d w=%d h=%d", panel_x, panel_y, panel_w,
               panel_h);
    DEBUG_INFO(MODULE_VIDEO, "  Panel inner: x=%d y=%d w=%d h=%d (border=%d)", inner_x, inner_y,
               inner_w, inner_h, border);
    DEBUG_INFO(MODULE_VIDEO, "  Avail area:  w=%d h=%d (after controls=%d)", inner_w, avail_h,
               CONTROLS_HEIGHT);
    DEBUG_INFO(MODULE_VIDEO, "  Video src:   %dx%d, fitted: %dx%d", _src_w, _src_h, fit_w, fit_h);
    DEBUG_INFO(MODULE_VIDEO, "  Centering:   x_offset=%d, y_offset=%d", x_offset, y_offset);
    DEBUG_INFO(MODULE_VIDEO, "  Video edges: left=%d right=%d top=%d bottom=%d", _screen_x,
               _screen_x + _video_w, _screen_y, _screen_y + _video_h);
    DEBUG_INFO(MODULE_VIDEO, "  Panel edges: left=%d right=%d top=%d bottom=%d", inner_x,
               inner_x + inner_w, inner_y, inner_y + avail_h);
    DEBUG_INFO(MODULE_VIDEO, "  SDL window:  x=%d y=%d (NOTE: Wayland always reports 0,0)", win_x,
               win_y);

    /* Query display info from compositor */
    {
        /* Try wayland-info (works on Weston and wlroots) */
        const char *cmds[] = {"wayland-info 2>/dev/null | grep -A5 'interface: wl_output'",
                              "weston-info 2>/dev/null | grep -A5 'interface: wl_output'",
                              "wlr-randr 2>/dev/null", NULL};
    ...
    lv_obj_set_flex_align(controls_bar, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
...
bool game_video_is_playing(void)
{
    if (video_pid > 0)
    {
        ...
        if (result == video_pid)
        {
            ...
            {
                DEBUG_WARN(MODULE_VIDEO, "ffplay exited unexpectedly with code %d",
                           WEXITSTATUS(status));
            }
            else if (WIFSIGNALED(status))
            {
